...
const { data, isLoading, refetch } = useQuery<SessionsApiResponse>({
    ...
    const prepareRows = data?.rows.map((item: SessionItem) => ({
        id: item.id,
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between flex-wrap gap-4">
                <h2 className="text-xl font-semibold">{t('sessions.title') || 'Sessions'}</h2>
                <select
