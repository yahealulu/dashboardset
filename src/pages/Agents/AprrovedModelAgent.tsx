...
const AprrovedModelAgent: FC<{
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: async () => {
            const res = await axios.post(`users/${id}/approve`);
            return res;
        },
        onSuccess: () => {
            toast.success(t('rejectAgents.toastSuccess2') as string);
            setOpen(false);
            refetch?.();
        },
...
export default AprrovedModelAgent;
